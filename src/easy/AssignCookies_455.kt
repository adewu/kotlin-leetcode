...
class AssignCookies_455 {
    ...
    fun findContentChildrenF(g: IntArray, s: IntArray): Int {
        var res = 0
        val gMap = HashMap<Int,Int>()
        g.forEachIndexed { index, i ->
            gMap[i] = gMap.getOrDefault(i,0) + 1
        }
        ...
        for ((k,v) in gMap){
            print("k:$k")
            println("sMap[k]Count:"+sMap.getOrDefault(k,0))
            res += Math.min(v,sMap.getOrDefault(k,0))
        }
        print("res:"+res)
        return res
    }
    ...
    fun findContentChildren(g: IntArray, s: IntArray): Int {
        ...
        var child = 0
        var cookie = 0
        while ((g.size > child) && (s.size > cookie)){
            if (g[child] <= s[cookie]){
                child++
            }
            cookie ++
